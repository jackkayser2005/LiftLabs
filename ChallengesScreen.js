...
import { updateDailyStreak } from './lib/streak'; // ← make sure this path is correct
import { addExp } from './lib/profile';           // ← same here
import { supabase } from './supabaseClient';
import styles from './styles';         // your existing styles
import popUpStyles from './popUpStyles'; // the file below
...
export default function ChallengesScreen({
  session,
  isPremium = false,
  onStreakUpdate,    // ← passed in from App.js
  isDarkMode = true,
}) {
  ...
  // Date string for today ‒ used to key AsyncStorage
  ...
  const handleComplete = async (id, premium = false) => {
    ...
    // Remove it from the local list so the UI updates
    ...
    // ** HERE’S WHERE WE BUMP THE “streak” **
    if (userId) {
      const newStreak = await updateDailyStreak(userId);
      if (typeof newStreak === 'number' && onStreakUpdate) {
        onStreakUpdate(newStreak);  // update top‐level state immediately
      }
    }

    // Reward logic (XP, level up, etc.) stays exactly the same
  ...
  const renderChallenge = (c, premium = false, locked = false) => (
    <View
      ...
      {!locked && !c.dbTracked && (
        <Pressable onPress={() => handleComplete(c.id, premium)} style={{ marginTop: 8 }}>
          {({ pressed }) => (
            <Text
              style={{
                color: pressed ? '#16a085' : '#1abc9c',
                textAlign: 'center',
              }}>
              Mark Complete
            </Text>
